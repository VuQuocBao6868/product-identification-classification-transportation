...
void PID(float error, Mode mode, int* FR, int* FL, int* RR, int* RL) {
  ...
  switch (mode) {
    ...
    case BACK_LEFT:
      *FR = (baseSpeed * 0.6) - (pid_output * 2);
      *FL = baseSpeed + abs(pid_output * 1.4);
      *RR = (baseSpeed * 0.6) - (pid_output * 2);
      *RL = baseSpeed + abs(pid_output * 1.4);
      break;
...
void loop() {
  ...
  // Apply motor speeds
  applyMotorSpeeds_Yellow(currentMode);
}
...
void condition_5() {
          ...
          if ((checked[0] == 0 && checked[1] == 0 && checked[2] == 0 && checked[3] == 0 && checked[4] == 1) || 
              (checked[0] == 0 && checked[1] == 0 && checked[2] == 0 && checked[3] == 1 && checked[4] == 1) || 
              (checked[0] == 0 && checked[1] == 0 && checked[2] == 1 && checked[3] == 1 && checked[4] == 1) || 
              (checked[0] == 1 && checked[1] == 0 && checked[2] == 0 && checked[3] == 0 && checked[4] == 0) || 
...
void applyMotorSpeeds_Red(Mode currentMode) {
  ...
  switch (currentMode) {
    ...
    case BACK_LEFT:
      m.motor(forn_right, BACKWARD,  FR);
      m.motor(forn_left,  BACKWARD,  FL);
      m.motor(rear_right, BACKWARD,  RR);
      m.motor(rear_left,  BACKWARD,  RL);
      break;
    ...
    case TURN_AROUND:
      m.motor(forn_right, BACKWARD,  80);
      m.motor(forn_left,  FORWARD,   80);
      m.motor(rear_right, BACKWARD,  80);
      m.motor(rear_left,  FORWARD,   80);    
      break;
...
void applyMotorSpeeds_Yellow(Mode currentMode) {
  ...
  switch (currentMode) {
    ...
    case BACK_WARD:
      m.motor(forn_right, BACKWARD, abs(FR));
      m.motor(forn_left,  BACKWARD, abs(FL));
      m.motor(rear_right, BACKWARD, abs(RR));
      m.motor(rear_left,  BACKWARD, abs(RL));
      break;
    ...
    case GO_SIDE:
      m.motor(forn_right, FORWARD, 140);
      m.motor(forn_left,  BACKWARD,140);
      m.motor(rear_right, BACKWARD,140);
      m.motor(rear_left,  FORWARD, 140);
      break;
    ...
    case TURN_AROUND:
      m.motor(forn_right, FORWARD, 80);
      m.motor(forn_left,  BACKWARD,80);
      m.motor(rear_right, FORWARD, 80);
      m.motor(rear_left,  BACKWARD,80);    
      break;

    case GO_AROUND:
      m.motor(forn_right, RELEASE,   0);
      m.motor(forn_left,  FORWARD,   100);
      m.motor(rear_right, BACKWARD,  110);
      m.motor(rear_left,  FORWARD,   110);
      break;
